/**
 * utils.ts — shared helpers across pages
 */
...
export function typeLabel(type: string, quizType?: string | null): string {
  if (type === "QUIZ" && quizType) {
    if (quizType === "PRE_LAB") return "Pre-lab";
    if (quizType === "POP")     return "Pop quiz";
    return "Quiz";
  }
...
/** Toggle assessment score between 0 and its weight */
...
export async function toggleScore(a: Assessment, onUpdate: (updated: Assessment) => void) {
  ...
  await api.assessments.update(a.assessment_id, {
    section_id: a.section_id, title: a.title, type: a.type,
    quiz_type: a.quiz_type, week_number: a.week_number,
    weight_percent: a.weight_percent, release_date: a.release_date,
    due_date: a.due_date, score: newScore,
  } as any);
