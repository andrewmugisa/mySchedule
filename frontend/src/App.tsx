...
const IconChevronL = () => <svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><polyline points="15 18 9 12 15 6"/></svg>;
const IconChevronR = () => <svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><polyline points="9 18 15 12 9 6"/></svg>;
...
export default function App() {
  ...
  return (
    <div style={{ display:"flex", height:"100vh", overflow:"hidden", background:"var(--bg)" }}>
      {/* Desktop sidebar */}
      <aside className="sidebar-transition hidden md:flex flex-col flex-shrink-0 overflow-hidden relative"
        style={{ width:sidebarW, background:"#0a0a0d", borderRight:"1px solid var(--border-dim)" }}>
        <SidebarContent collapsed={collapsed} courses={courses} />
        <button onClick={() => setCollapsed(c => !c)} className="btn-icon"
          style={{ position:"absolute", top:18, right:-14, width:26, height:26, borderRadius:"50%", background:"var(--bg-card2)", border:"1px solid var(--border)", boxShadow:"0 2px 8px rgba(0,0,0,0.4)", zIndex:20 }}
