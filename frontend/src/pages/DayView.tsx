...
function EventPopover({ event, color, label, onClose, onCancel, onDelete }: {
  ...
  useEffect(() => {
    function onMove(e: MouseEvent) {
      ...
      const popH = ref.current?.offsetHeight ?? 260;
      setPos({
        x: Math.max(8, Math.min(window.innerWidth  - popW - 8, e.clientX - dragOffset.current.x)),
        y: Math.max(8, Math.min(window.innerHeight - popH - 8, e.clientY - dragOffset.current.y)),
      });
    }
    ...
    >
      ...
      >
        ...
        <div style={{ flex:1, minWidth:0 }}>
          ...
          <div style={{ fontSize:14, fontWeight:600, color: cancelled ? "var(--text-3)" : "var(--text)", textDecoration: cancelled ? "line-through" : "none", lineHeight:1.3 }}>
            {event.title}
          </div>
          <div style={{ fontSize:11, color:"var(--text-3)", marginTop:3 }}>
            {fmtTime12(new Date(event.start_time))} – {fmtTime12(new Date(event.end_time))}
          </div>
      ...
      <div style={{ padding:"8px 10px" }}>
        {isRecurring ? (
          <div style={{ fontSize:12, color:"var(--text-3)", padding:"6px 4px" }}>
            Recurring event — edit from <strong style={{ color:"var(--text-2)" }}>Settings</strong>.
          </div>
        ) : (
          <>
            <button onClick={onCancel}
              style={{ width:"100%", textAlign:"left", padding:"8px 10px", borderRadius:8, fontSize:12, fontWeight:500, cursor:"pointer", background:"none", border:"none", color: cancelled ? "var(--green)" : "var(--orange)", fontFamily:"inherit", transition:"background 0.15s" }}
              onMouseEnter={e=>(e.currentTarget.style.background="var(--bg-hover)")}
              onMouseLeave={e=>(e.currentTarget.style.background="none")}>
              {cancelled ? "↩ Restore to schedule" : "⊘ Cancel this event"}
            </button>
            <button onClick={onDelete}
              style={{ width:"100%", textAlign:"left", padding:"8px 10px", borderRadius:8, fontSize:12, fontWeight:500, cursor:"pointer", background:"none", border:"none", color:"var(--red)", fontFamily:"inherit", transition:"background 0.15s" }}
...
export default function DayView() {
  ...
  async function cancelEvent(ev:Event){await api.events.update(ev.event_id,{...ev,is_cancelled:!ev.is_cancelled});setEvents(prev=>prev.map(e=>e.event_id===ev.event_id?{...e,is_cancelled:!e.is_cancelled}:e));setOpenId(null);}
  async function deleteEvent(ev:Event){if(!confirm("Delete this event permanently?"))return;await api.events.delete(ev.event_id);setEvents(prev=>prev.filter(e=>e.event_id!==ev.event_id));setOpenId(null);}

  const spineSlots=useMemo(()=>Array.from({length:TOTAL_SLOTS+1},(_,i)=>{const totalMins=START_HOUR*60+i*SLOT_MINS,h=Math.floor(totalMins/60),m=totalMins%60,isHour=m===0;const label=isHour?`${h===12?12:h>12?h-12:h}${h>=12?"PM":"AM"}`:"";return{label,isHour,topPx:i*SLOT_PX};}),[]);
  ...
  // Find the event currently open (for rendering popover)
  ...
  return(
    <div style={{display:"flex",height:"100%",overflow:"hidden"}}>
              ...
              {weekStrip.map((d,i)=>{
                ...
                return(
                  <button key={i} onClick={()=>setSelected(new Date(d))}
                    style={{display:"flex",flexDirection:"column",alignItems:"center",padding:"5px 5px",borderRadius:9,cursor:"pointer",border:"none",
                      background:isSel?"var(--accent)":"transparent",
                      transition:"background 0.15s",minWidth:34,fontFamily:"inherit"}}
          ...
          <div style={{position:"relative",height:totalH}}>
            ...
            {spineSlots.map(({label,isHour,topPx},i)=>(
              <div key={i} style={{position:"absolute",top:topPx,left:0,right:0,display:"flex",alignItems:"center",height:1}}>
                <div style={{width:50,textAlign:"right",paddingRight:7,flexShrink:0}}>
                  {isHour&&<span style={{fontSize:9,fontWeight:600,color:"var(--text-4)",letterSpacing:"0.02em"}}>{label}</span>}
                </div>
            ...
            <div style={{position:"absolute",left:78,right:4,top:0,bottom:0}}>
              {laid.map(({ev,col,cols})=>{
                ...
                const heightPx=Math.max((endMins-startMins)/SLOT_MINS*SLOT_PX,22);
                ...
                return(
                  <div key={ev.event_id} style={{position:"absolute",top:topPx,height:heightPx,width:colW,left:leftPct,zIndex:isOpen?30:1}}>
                    <div
                      ...
                      style={{width:"100%",height:"100%",borderRadius:7,borderLeft:`3px solid ${cancelled?"var(--border-strong)":color}`,
                        background:personal?"rgba(255,255,255,0.03)":`${color}14`,
                        opacity:cancelled?0.4:1,cursor:"pointer",padding:"4px 7px",
                        position:"relative",overflow:"hidden",transition:"filter 0.15s,transform 0.1s"}}
                      ...
                      {/* Time badge */}
                      <div style={{position:"absolute",top:4,right:5,fontSize:8,fontWeight:600,color:personal?"var(--text-4)":`${color}bb`,background:personal?"var(--bg-hover)":`${color}18`,border:`1px solid ${personal?"var(--border-dim)":`${color}28`}`,borderRadius:4,padding:"1px 4px",whiteSpace:"nowrap"}}>
      ...
      {/* Floating centered draggable popover — rendered outside column flow */}
      {openId!=null&&openEvent&&(
        <EventPopover
          event={openEvent.ev}
          color={evColor(openEvent.ev)}
          label={evLabel(openEvent.ev)}
